...
func TestRunCompletion(t *testing.T) {
	tests := []struct {
		...
		{"zsh", []string{"zsh"}, "#compdef minikube-support\n", false},
		{"bash", []string{"bash"}, "# bash completion for minikube-support", false},
	}
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			out := &bytes.Buffer{}
			rCmd := &cobra.Command{Use: "minikube-support"}
...
func Test_runCompletionZsh(t *testing.T) {
	tests := []struct {
		...
		{"ok", &bytes.Buffer{}, "#compdef minikube-support\n", false},
